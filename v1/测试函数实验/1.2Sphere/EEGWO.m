...
for Exper_index = 1:Exper_num
    ...
    while iter <= ger
        ...
        factor = 2-(2-0)*((ger-iter)/ger)^1.5;%随机因子
        ...
        for i=1:sizepop
            ...
            wolf_rand_index_2 = randperm(sizepop,2);%随机选择一个个体
            for z=1:2
                if wolf_rand_index_2(1,z)~= i
                    flag = z;%标记第几个
                    break;
                end
            end
            wolf_rand = wolf_pop(:,wolf_rand_index_2(1,flag));
            
            r3 = rand(dimension,1);
            r4 = rand(dimension,1);
            
            pop_new(:,i) = 0.1 * r3 .* (X1 + X2 + X3)/3 + 0.9 * r4 .*(-wolf_pop(:,i) + wolf_rand);
...
%求20次实验平均的适应值并保存
EEGWO_aver_fitness = sum(Exper_best_fitness_ger,1)/Exper_num;
save EEGWO_aver_fitness.mat EEGWO_aver_fitness;


%保存20组最好的适应值 画线形图
EEGWO_best_fitness = Exper_best_fitness_arr;
save EEGWO_best_fitness.mat  EEGWO_best_fitness ;
