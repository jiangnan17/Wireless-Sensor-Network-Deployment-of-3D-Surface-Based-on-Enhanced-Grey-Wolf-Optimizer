...
for Exper_index = 1:Exper_num
    ...
    while iter <= ger
        ...
        for i=1:sizepop
            ...
            C1 = 2 * rand(dimension,1) - 2/3 * factor;
            C2 = 2 * rand(dimension,1) - 2/3 * factor;
            C3 = 2 * rand(dimension,1) - 2/3 * factor;
            ...
            %越界处理
            for k=1:dimension
                if  (X_1(k,1) > limit(1,2))|| (X_1(k,1) < limit(1,1))
                    X_1(k,1) = limit(1,1) + rand(1,1)*(limit(1,2) - limit(1,1));
                end  
            end
            
            X_1_fitness = func(X_1);
            %反向学习
            r4 = rand(dimension,1);
            X = limit(1,1) + limit(1,2) - r4.*X_1;
            %越界处理
            for k=1:dimension
                if  (X(k,1) > limit(1,2))|| (X(k,1) < limit(1,1))
                    X(k,1) = limit(1,1) + rand(1,1)*(limit(1,2) - limit(1,1));
                end  
            end
...
%求20次实验平均的适应值并保存
ROLGWO_aver_fitness = sum(Exper_best_fitness_ger,1)/Exper_num;
save ROLGWO_aver_fitness.mat ROLGWO_aver_fitness;


%保存20组最好的适应值 画线形图
ROLGWO_best_fitness = Exper_best_fitness_arr;
save ROLGWO_best_fitness.mat  ROLGWO_best_fitness ;
